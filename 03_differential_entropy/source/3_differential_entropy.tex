...
explicitely work out the difference. Consider a probability density
...
has two componentsthe entropy: the entropy of the $N$-bit encoding of
the result and another term corresponding to what remains after the
$N$-bit encoding.

\subsection*{Changes fo variable}
...
\begin{equation}
  P(x\in[x_0,x_1))=\int__{x_0}^{x_1}p_X(x)dx
\end{equation}
...
\begin{equation}
  h(Y)=h(X)+\int p_X(x) \log_2\left|\frac{dx}{dy}\right|
\end{equation}
...
something we observed whenw we calculated the differential entropy for
the uniform distribution and the Gaussian.
